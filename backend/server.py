...
# --- SYSTEM CONFIGURATION (production: env vars only) ---
...
FRONTEND_BASE_URL = os.getenv("FRONTEND_BASE_URL", "https://projectdeployment-khaki.vercel.app")

# Publicly allowed CORS origins (comma-separated env var)
DEFAULT_ORIGINS = [
    "https://projectdeployment-khaki.vercel.app",
...
# --- DATABASE CONFIGURATION (production: env vars only) ---
...
# Root route — prevents the black-screen / {"detail":"Not Found"} confusion
@app.get("/")
def root():
    return {
        "status": "ok",
...
def register_user(payload: RegisterRequest, db: Session = Depends(get_db)):
    ...
    # Neutral, public shareable link (NO t.me bot link)
...
def paynow_webhook(payload: PaynowWebhookRequest, db: Session = Depends(get_db)):
    ...
    if not deposit:
        deposit = Deposit(telegram_id=payload.telegram_id, transaction_id=payload.reference,
...
def request_withdrawal(payload: WithdrawalRequest, db: Session = Depends(get_db)):
    ...
    db.add(Withdrawal(telegram_id=payload.telegram_id, amount=payload.amount, method=payload.method,
                      account_details=payload.account_details, status="Pending"))
    db.commit()
...
async def approve_deposit(deposit_id: int, admin: str = Depends(get_current_admin), db: Session = Depends(get_db)):
    ...
    if deposit.status == "Approved":
        raise HTTPException(status_code=400, detail="Deposit is already approved.")
